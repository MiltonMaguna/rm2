...
from rm2.render_manager.core.disk_collector import (
    _get_render_layer_names,
    _get_last_version_path,
)
from rm2.render_manager.core.disk_collector import _get_valid_render_layers, _get_aovs
from rm2.render_manager.core.disk_collector import check_for_empty_subfolders
from rm2.render_manager.core.disk_collector import check_for_files_exr
...
def test_collector(shot_render_frames_path):
    ...
    empty = check_for_empty_subfolders(
        os.path.join(shot_render_frames_path, "RND_MG_BAD")
    )
    ...
    version = _get_last_version_path(
        os.path.join(shot_render_frames_path, "RND_FG_BTY")
    )
    assert version == "I:/GizmoRD/FRAMES/PYTEST/030/CG/RND_FG_BTY/LGT_KAF_010_v0026"

    version = _get_last_version_path(
        os.path.join(shot_render_frames_path, "RND_FG_BAD")
    )
    assert version is None

    aovs = _get_aovs(
        name="RND_FG_BTY",
        path=os.path.join(shot_render_frames_path, "RND_FG_BTY", "LGT_KAF_010_v0026"),
    )
    assert aovs == [
        "AO",
        "beauty",
