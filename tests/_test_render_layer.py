...
def test_collector_and_render_layer(shot_render_frames_path, mocked_nuke_no_nodes):
    ...
    assert (
        render.path() == "I:/GizmoRD/FRAMES/PYTEST/030/CG/RND_FG_BTY/LGT_KAF_010_v0026"
    )
    assert render.aovs() == [
        "AO",
        "beauty",
...
def test_collector_and_render_layer_in_sync(
    shot_render_frames_path, read_node_same_version
):
    renders = collect_render_layers_from(shot_render_frames_path)
    render = renders[1] if renders else None
...
def test_collector_and_render_layer_outdated(
    shot_render_frames_path, read_node_outdated_version
):
    renders = collect_render_layers_from(shot_render_frames_path)
    render = renders[1] if renders else None
...
def test_remove_render_layer_with_nodes(
    shot_render_frames_path, read_node_same_version
):
    renders = collect_render_layers_from(shot_render_frames_path)
    render = renders[1] if renders else None
