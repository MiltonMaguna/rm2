...
from RenderManager2.render_manager2.render.libs.helpers.config import (
    BACKDROP_SIZE,
    COLOR_BACKDROP_RL,
    OFFSET_BDROP_X,
    OFFSET_BDROP_Y,
...
def get_max_row_backrops(same_name_backdrops: list) -> int:
    ...
    if same_name_backdrops:
        ...
        max_row = max(int(backdrop['row'].getValue()) for backdrop in same_name_backdrops)
        return max_row + 1
    else:
        return 1
...
def _create_backdrop(backdrop_name: str, backdrop_size: dict, subcontainer: bool = False):
