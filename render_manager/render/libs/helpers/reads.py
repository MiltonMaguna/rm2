...
from rm2.render_manager.render.libs.helpers.config import POSITION_READ, NODE_CUSTOM
from rm2.render_manager.render.libs.helpers.config import ANCHOR_READ, OFFSET_READ_X
from rm2.render_manager.render.libs.helpers.config import (
    BACKDROP_SIZE,
    OFFSET_BORDER_BACKDROP,
)
...
def create_node(render: Render, aov_name: str, pos_read_x: int, pos_read_y: int) -> int:
    ...
    file = (
        f"{render.path()}/{aov_name}/{aov_data['files']}_####.{aov_data['extension']}"
    )
