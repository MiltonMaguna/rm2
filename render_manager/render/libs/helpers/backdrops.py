...
from render_manager.render.libs.helpers.config import BACKDROP_SIZE, ROL_POSITION_X
from render_manager.render.libs.helpers.config import OFFSET_BDROP_X, OFFSET_BDROP_Y
from render_manager.render.libs.helpers.config import COLOR_BACKDROP_RL
...
def create_backdrop_container(render: Render, row: int) -> str:
    ...
    for n in nuke.allNodes('BackdropNode'):
        with contextlib.suppress(NameError):
            if n['rol_layer'].getValue() == render.rol_layer() and n['container'].getValue():
                return n
...
def create_backdrop_subcontainer(render: Render, row: int):
    ...
    for bd in nuke.allNodes('BackdropNode'):
        with contextlib.suppress(NameError):
            if bd["name_layer"].getValue() == render.name() and bd['subcontainer'].getValue():
...
def _add_attributes_tab(node, render: Render, row: int, container: bool = False) -> None:
...
def _create_backdrop(backdrop_name: str, backdrop_size: dict):
    ...
    node = nuke.nodes.BackdropNode(label=backdrop_name,
